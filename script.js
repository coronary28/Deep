...
function applyThemeUI(){ const t=theme(); if(el('nav-icon-exams')) el('nav-icon-exams').textContent=t.icons.exams; if(el('nav-icon-wrong')) el('nav-icon-wrong').textContent=t.icons.wrong; if(el('nav-icon-favorites')) el('nav-icon-favorites').textContent=t.icons.favorites; if(el('nav-icon-checklist')) el('nav-icon-checklist').textContent=t.icons.checklist || '☑️'; if(el('nav-icon-search')) el('nav-icon-search').textContent=t.icons.search; if(el('nav-icon-statistics')) el('nav-icon-statistics').textContent=t.icons.statistics; if(el('nav-icon-settings')) el('nav-icon-settings').textContent=t.icons.settings; if(el('nav-icon-memories')) el('nav-icon-memories').textContent='📖'; if(el('statistics-screen-title')) el('statistics-screen-title').textContent=t.texts.statsTitle; if(el('settings-title')) el('settings-title').textContent=t.texts.settingsTitle; if(el('exam-settings-title')) el('exam-settings-title').textContent=t.texts.examSettingsTitle; if(el('btn-exam-settings')) el('btn-exam-settings').textContent=t.texts.examSettingsButton; if(el('btn-start-exam')) el('btn-start-exam').textContent=t.texts.startExam; const a=document.querySelector('#btn-training-mode .mode-label'); const b=document.querySelector('#btn-exam-mode .mode-label'); if(a) a.textContent=t.texts.trainingLabel; if(b) b.textContent=t.texts.examLabel; }
...
function applyEffectAudioVolumes(){ ['right-audio','wrong-audio','celebrate-audio','seconds-audio'].forEach(id=>{ const a=el(id); if(a) a.volume=(state.settings.volume||50)/100; }); }
async function prepareStaticEffectAudio(){ const right = el('right-audio'); const wrong = el('wrong-audio'); const celebrate = el('celebrate-audio'); const seconds = el('seconds-audio'); if(right) right.src=await resolveAssetPath(['right.mp3','audio/right.mp3','assets/audio/right.mp3']); if(wrong) wrong.src=await resolveAssetPath(['wrong.mp3','audio/wrong.mp3','assets/audio/wrong.mp3']); if(celebrate) celebrate.src=await resolveAssetPath(['celebrate.mp3','audio/celebrate.mp3','assets/audio/celebrate.mp3']); if(seconds) seconds.src=await resolveAssetPath(['seconds.mp3','audio/seconds.mp3','assets/audio/seconds.mp3']); applyEffectAudioVolumes(); }
...
function playCelebrateSound(){ if(state.settings.feedbackEnabled===false) return; const a=el('celebrate-audio'); if(!a||!a.src) return; try{ a.currentTime=0; a.play().catch(()=>{}); }catch(e){} }
function playSecondsSound(){ if(state.settings.feedbackEnabled===false) return; const a=el('seconds-audio'); if(!a||!a.src) return; try{ a.currentTime=0; a.play().catch(()=>{}); }catch(e){} }
...
function renderGlobalStats(){
  ...
  container.innerHTML = `
    <div class="progress-card">
      ...
      <p style="margin-top:8px;"><span>نسبة الإنجاز</span><strong>${percentage}%</strong></p>
  ...
  container.innerHTML = subjectsToShow.map(subject => {
    ...
    return `
      <div class="stats-subject-card" onclick="openSubjectStats('${subject.id}')">
        ...
        <div class="stats-row" style="margin-top:4px;"><span>نسبة الإنجاز</span><strong>${pct}%</strong></div>
      </div>
    `;
  }).join('');
}
function renderSectionAnalyticsCard(subject, type, label, icon, analytics){
  ...
  const rowsHtml = analytics.rows.length ? analytics.rows.map(row => `
    <div class="stats-lecture-row" style="--subject-color:${getSubjectColor(subject.name)}">
      ...
      <div class="stats-lecture-progress">
        ...
        <div class="stats-row" style="margin-top:4px;"><span>نسبة الإنجاز</span><strong>${row.percentage}%</strong></div>
      </div>
    </div>
  ...
  return `
    <div class="stats-section-card stats-section-accent" style="--section-accent:${palette.accent};--section-accent-bg:${palette.bg};">
      ...
      <div style="margin-top:4px; text-align:right;"><span style="color:var(--text-light);">نسبة الإنجاز</span> <strong>${analytics.percentage}%</strong></div>
  ...
  if(el('subject-stats-summary')){
    el('subject-stats-summary').innerHTML = `
      <div class="progress-card">
        ...
        <p style="margin-top:8px;"><span>نسبة الإنجاز</span><strong>${pct}%</strong></p>
        <div class="progress-bar"><span style="width:${pct}%"></span></div>
      </div>
    `;
  }
...
function renderSubjects(){
  ...
  const hint = el('exams-hint-bar'); if(hint) hint.classList.toggle('hidden', false);
...
function startExamSession(questions,mode,direction,sourceLabel,extraMinutes,meta){
  const numberedQuestions = questions.map((q, idx) => ({ ...q, displayNumber: idx + 1, originalNumber: q.number }));
  state.currentExam={ mode, direction, sourceLabel:sourceLabel||'custom', collectionType: meta && meta.collectionType ? meta.collectionType : null, displayLabel: meta && meta.displayLabel ? meta.displayLabel : (sourceLabel||'Exam'), historySubjectName: meta?.historySubjectName || null, historyGroups: meta?.historyGroups || [], questions:numberedQuestions, currentIndex:0, answers:new Array(numberedQuestions.length).fill(null), firstAnswers:new Array(numberedQuestions.length).fill(null), startTime:Date.now(), totalTime:(numberedQuestions.length+(extraMinutes||0))*60*1000, submitted:false, showAnswer:false, masteredWrongIds:[], secondsSoundPlayed:false }; saveExamState(); showScreen('exam-screen'); renderExam(); if(mode==='exam') startTimer(); scrollQuestionIntoView(false); }
...
function renderExam(){
  ...
  const correctCount = state.currentExam.answers.reduce((sum, ans, i) => sum + (ans !== null && isAnswerCorrect(questions[i], ans) ? 1 : 0), 0);
  const percentage = answeredCount > 0 ? Math.round((correctCount / answeredCount) * 100) : 0;
  const badgeText = `🎯 ${correctCount}/${total} · ✅${correctCount} · ${percentage}%`;
  if(el('exam-stats-badge')) el('exam-stats-badge').textContent = badgeText;
  if(state.currentExam.mode === 'exam'){
    if(el('exam-timer')) el('exam-timer').classList.remove('hidden');
    if(el('exam-timer-far')) el('exam-timer-far').classList.remove('hidden');
  } else {
    if(el('exam-timer')) el('exam-timer').classList.add('hidden');
    if(el('exam-timer-far')) el('exam-timer-far').classList.add('hidden');
  }
...
function renderGrid(){
  ...
  for(let i = questions.length-1; i >= 0; i--){
    const q = questions[i];
    let cls='grid-btn';
    if(i===state.currentExam.currentIndex) cls+=' current';
    else if(state.currentExam.answers[i]!==null){
      if(state.currentExam.mode==='training' && state.currentExam.firstAnswers[i]!==null) cls+=isAnswerCorrect(q,state.currentExam.firstAnswers[i])?' answered':' wrong';
      else cls+=' answered';
    }
    if(state.currentExam.direction==='oneway' && i<state.currentExam.currentIndex) cls+=' disabled';
...
function selectOption(optionIndex){
  ...
  if(state.currentExam.mode==='training'){
    ...
    renderExam();
    renderGrid();
    const badge = el('exam-stats-badge');
    if(badge){
      const total = state.currentExam.questions.length;
      const correctCount = state.currentExam.answers.reduce((sum, ans, i) => sum + (ans !== null && isAnswerCorrect(state.currentExam.questions[i], ans) ? 1 : 0), 0);
      const answeredCount = state.currentExam.answers.filter(a => a !== null).length;
      const percentage = answeredCount > 0 ? Math.round((correctCount / answeredCount) * 100) : 0;
      badge.textContent = `🎯 ${correctCount}/${total} · ✅${correctCount} · ${percentage}%`;
    }
  }
}
...
function toggleQuestionLocation(){ if(!state.currentExam) return; const q=state.currentExam.questions[state.currentExam.currentIndex]; const parts=[]; if(q.subjectName) parts.push('المادة: '+q.subjectName); if(q.lectureName) parts.push('الملف: '+q.lectureName); if(q.batchName) parts.push('الدفعة: '+q.batchName); if(q.originalNumber) parts.push('الرقم الأصلي: '+q.originalNumber); if(q.pageNumber) parts.push('الصفحة: '+q.pageNumber); showToast(parts.join(' | ') || 'لا توجد بيانات موقع متاحة.','info',3000); }
...
function exitExam(){
  if(!state.currentExam) { goHome(); return; }
  if(state.currentExam.submitted){
  ...
  if(actionsDiv){
    const existingSaveBtn = actionsDiv.querySelector('.dialog-save-btn');
    if(existingSaveBtn) existingSaveBtn.remove();
    const saveBtn = document.createElement('button');
    saveBtn.className = 'btn-secondary dialog-save-btn';
...
function startTimer(){
  clearInterval(state.timerInterval);
  const timerElFar = el('exam-timer-far');
  const timerEl = el('exam-timer');
  const showTimer = (el) => { if(el) el.classList.remove('hidden'); };
  showTimer(timerElFar);
  showTimer(timerEl);
  state.timerInterval = setInterval(()=>{
    ...
    if(timerElFar) timerElFar.textContent = timeString;
    if(timerEl) timerEl.classList.toggle('timer-danger', remaining <= 60000);
    if(timerElFar) timerElFar.classList.toggle('timer-danger', remaining <= 60000);
    if(remaining <= 11000 && remaining > 0 && !state.currentExam.secondsSoundPlayed){
...
function recordExamMemory(){
  ...
  state.currentExam.questions.forEach((q,idx)=>{
    ...
    if(!existing){
      ...
      const existingDate = new Date(existing.ts).toDateString();
      const newDate = new Date(end).toDateString();
...
function finishExam(){
  if(!state.currentExam) return;
  const unansweredIndices = state.currentExam.answers.reduce((arr, ans, idx) => { if(ans === null) arr.push(idx); return arr; }, []);
  if(state.currentExam.mode !== 'training' && unansweredIndices.length > 0){
    askConfirm(`هناك ${unansweredIndices.length} سؤال لم تحلهم ، هل تريد تأكيد تسليم الاختبار دون حلهم؟`, () => {
      finalizeExam();
    }, () => {});
    return;
  }
...
function saveProgress(){
  ...
  updateStatisticsIfOpen();
  renderMemories();
  checkAndAutoCompleteLectures();
}
function checkAndAutoCompleteLectures(){
  if(!state.currentExam) return;
  const affectedGroups = new Map();
  state.currentExam.questions.forEach(q => {
    const groupKey = `${q.subjectName}|${q.lectureName}|${q.sourceType}`;
    if(!affectedGroups.has(groupKey)){
      affectedGroups.set(groupKey, { subjectName: q.subjectName, lectureName: q.lectureName, sourceType: q.sourceType || 'lecture' });
    }
  });
  for(const groupInfo of affectedGroups.values()){
    const subject = state.subjects.find(s => s.name === groupInfo.subjectName);
    if(!subject) continue;
    let targetGroup = null;
    if(groupInfo.sourceType === 'lecture') targetGroup = subject.lectures.find(g => g.name === groupInfo.lectureName);
    else if(groupInfo.sourceType === 'ai') targetGroup = subject.ai.find(g => g.name === groupInfo.lectureName);
    if(!targetGroup) continue;
    const total = targetGroup.questions.length;
    const answered = getSubjectProgressEntryForGroup(targetGroup);
    if(total > 0 && answered === total && !state.checklistCompleted[targetGroup.id]){
      state.checklistCompleted[targetGroup.id] = true;
      saveChecklistStore();
      moveGroupToBottomByInfo(subject.name, groupInfo.sourceType, targetGroup.id);
      showToast(`✅ تم إكمال ${targetGroup.name} بالكامل! تم نقله للأسفل.`, 'success', 3000);
    }
  }
  renderChecklist();
  if(el('checklist-subject-screen') && el('checklist-subject-screen').classList.contains('active')) renderChecklistSubject();
  if(el('subject-sections-screen') && el('subject-sections-screen').classList.contains('active') && state.currentSubject) openSubject(state.currentSubject.id);
}
...
function getGroupOrderKey(subjectName, sectionType){ return `${subjectName || 'unknown'}::${normalizeSectionType(sectionType)}`; }
function normalizeSectionType(sectionType){ const s = String(sectionType || '').toLowerCase(); if(s === 'lecture' || s === 'lectures') return 'lectures'; if(s === 'year' || s === 'years') return 'years'; if(s === 'ai') return 'ai'; return s || 'custom'; }
...
function saveGroupPreferences(){ try{ localStorage.setItem('medical-app-group-prefs-v2', JSON.stringify(state.groupPreferences || {})); }catch(e){} }
...
function aggregateMemorySeries(period, detailed, selectedSubjects, offset = 0){
  ...
  if(period==='weekly'){
    const baseStart = getStartOfWeek(now);
    rangeStart = new Date(baseStart);
    rangeStart.setDate(baseStart.getDate() + (offset * 7));
    rangeEnd = getEndOfWeek(rangeStart);
    caption = `من ${rangeStart.toLocaleDateString('ar-EG')} إلى ${rangeEnd.toLocaleDateString('ar-EG')}`;
    for(let i=0;i<7;i++){ const d=new Date(rangeStart); d.setDate(rangeStart.getDate()+i); const key=d.toISOString().slice(0,10); buckets.push(key); labels.push(d.toLocaleDateString('ar-EG',{weekday:'short', day:'numeric'})); }
  } else if(period==='monthly'){
    const baseStart = getStartOfMonth(now);
    rangeStart = new Date(baseStart);
    rangeStart.setMonth(baseStart.getMonth() + offset);
    rangeEnd = getEndOfMonth(rangeStart);
    caption = `${rangeStart.toLocaleDateString('ar-EG')} — ${rangeEnd.toLocaleDateString('ar-EG')}`;
    const days = rangeEnd.getDate();
    for(let i=1;i<=days;i++){ const d=new Date(rangeStart.getFullYear(), rangeStart.getMonth(), i); const key=d.toISOString().slice(0,10); buckets.push(key); labels.push(String(i)); }
  } else {
    const baseStart = getStartOfYear(now);
    rangeStart = new Date(baseStart);
    rangeStart.setFullYear(baseStart.getFullYear() + offset);
    rangeEnd = getEndOfYear(rangeStart);
    caption = `${rangeStart.toLocaleDateString('ar-EG')} — ${rangeEnd.toLocaleDateString('ar-EG')}`;
    for(let i=0;i<12;i++){ const d=new Date(rangeStart.getFullYear(), i, 1); const key=d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0'); buckets.push(key); labels.push(d.toLocaleDateString('ar-EG',{month:'short'})); }
  }
  ...
  if(!detailed){ const data = buckets.map(key=>filteredEntries.filter(e=>getBucketKey(e.ts)===key).length); const maxVal = Math.max(1, ...data); const adjustedData = data.map(v => v === 0 ? 0 : v); return {labels, series:[{name:'كل المواد', data:adjustedData, color:'#2563eb'}], caption, maxVal}; }
  ...
  return {labels, series, caption, maxVal};
}
function drawMemoriesChart(canvas, labels, series, maxVal){
  ...
  series.forEach((s, idx)=>{
    ...
    s.data.forEach((value, i)=>{
      const x=left + step*i;
      const y=h-bottom - (value/maxVal)*plotH;
  ...
  if(series.length > 1){
    ...
    ctx.textAlign='left';
    series.forEach((s, idx)=>{
      const x = left + (idx % 4) * 140;
      const y = legendY - (Math.floor(idx / 4) * 20);
      ctx.fillStyle=s.color;
      ctx.fillRect(x, y-10, 18, 4);
      ctx.fillStyle='#475569';
      ctx.fillText(s.name, x+24, y-6);
    });
  }
}
...
function renderMemories(){
  ...
  const canvas = el('memories-chart'); if(canvas) drawMemoriesChart(canvas, data.labels, data.series, data.maxVal);
  ...
  setupChartNavButtons(period);
}
function setupChartNavButtons(period){
  const prevBtn = document.getElementById('chart-prev-btn');
  const nextBtn = document.getElementById('chart-next-btn');
  if(prevBtn){
    prevBtn.onclick = () => { state.chartPeriodOffset = (state.chartPeriodOffset || 0) - 1; renderMemories(); };
  }
  if(nextBtn){
    nextBtn.onclick = () => { state.chartPeriodOffset = (state.chartPeriodOffset || 0) + 1; renderMemories(); };
  }
}
...
function setupLogos(){
  const container = el('logo-container');
  if(!container) return;
  const logoUrls = ['logo.jpg', 'logo2.jpg', 'logo3.jpg'].map(name => name);
  ...
  Promise.all(logoUrls.map(url => checkImage(url))).then(results => {
    const available = results.filter(r => r !== null);
    if(available.length === 0){
      container.innerHTML = '<div style="font-size:3rem; margin-bottom:10px;">🩺</div><div id="random-quote" class="app-subtitle"></div>';
      return;
    }
    let html = '<div style="display: flex; justify-content: center; align-items: center; gap: 16px; flex-wrap: wrap; margin-bottom: 12px;">';
    if(available.length === 1){
      html += `<img src="${available[0]}" alt="Logo" style="max-width: 150px; height: auto; border-radius: 14px; box-shadow: var(--shadow); border: 1px solid var(--border);">`;
    } else if(available.length === 2){
      const [first, second] = available;
      html += `<img src="${first}" alt="Logo" style="max-width: 120px; height: auto; border-radius: 14px; box-shadow: var(--shadow); border: 1px solid var(--border);">`;
      html += `<img src="${second}" alt="Logo2" style="max-width: 120px; height: auto; border-radius: 14px; box-shadow: var(--shadow); border: 1px solid var(--border);">`;
    } else {
      const [logo, logo2, logo3] = available;
      html += `<img src="${logo2}" alt="Logo2" style="max-width: 100px; height: auto; border-radius: 14px; box-shadow: var(--shadow); border: 1px solid var(--border);">`;
      html += `<img src="${logo}" alt="Logo" style="max-width: 120px; height: auto; border-radius: 14px; box-shadow: var(--shadow); border: 1px solid var(--border);">`;
      html += `<img src="${logo3}" alt="Logo3" style="max-width: 100px; height: auto; border-radius: 14px; box-shadow: var(--shadow); border: 1px solid var(--border);">`;
    }
    html += '</div><div id="random-quote" class="app-subtitle"></div>';
